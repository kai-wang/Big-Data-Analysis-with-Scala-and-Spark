...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val leisure = new ListBuffer[Column]()
    ...
    for(c <- columnNames) {
      ...
      || c.startsWith("t15") || c.startsWith("t16") || c.startsWith("t18"))
        leisure += col(c)
    }
  ...
  def timeUsageSummary(
    ...
    implicit def colToString(column: Column): String = column.toString()

    def sumall(columns: List[Column], sum: Column): Column = columns match {
      case Nil => sum
      case x::xs => sumall(xs, sum + df(x))
    }

    val primaryNeedsProjection: Column = (sumall(primaryNeedsColumns, lit("0.0d")) / 60.0d).as("primaryNeeds")
    val workProjection: Column = (sumall(workColumns, lit("0.0d")) / 60.0d).as("work")
    val otherProjection: Column = (sumall(otherColumns, lit("0.0d")) / 60.0d).as("other")
